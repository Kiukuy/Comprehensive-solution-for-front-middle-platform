...
onMounted(() => {
  watch(
    ...
    () => {
      if (
        containerTarget.value.offsetWidth === 1280 &&
        columnWidth.value === 240
      )
        return
      useColumnWidth()
    },
...
const waitImgComplete = () => {
  itemHeights = []
  // 拿到所有元素
  let itemElemets = [...document.getElementsByClassName('m-waterfall-item')]
  // 获取所有 img 标签
  const imgElements = getImgElements(itemElemets)
  // 获取所有 img 标签的图片
  const allImgs = getAllImg(imgElements)
  onCompleteImgs(allImgs).then(() => {
    // 图片加载完成，获取高度

    itemElemets.forEach((el) => {
...
watch(columnWidth, () => {
  ...
  // 重新构建高度记录器
  useColumnHeightObj()
  // 清除 item._style 否则无法重新计算 left 和 top
